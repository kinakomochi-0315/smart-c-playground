...
test("main.cとC17 compile_flags.txtを作成して一回限りチケットを発行する", async () => {
    ...
    const mainPath = fileURLToPath(response.documentUri);
    assert.equal(await readFile(mainPath, "utf8"), "int main(void) { return 0; }\n");
    assert.equal(
...
test("sandbox利用時はブラウザへ/workspace/main.cだけを公開する", async () => {
    ...
    assert.equal(response.documentUri, "file:///workspace/main.c");
    await manager.shutdown();
});
